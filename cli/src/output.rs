...
fn format_service_trace_browser_capability_launch_text(data: &serde_json::Value) -> Option<String> {
    let launches = data
        .get("summary")
        .and_then(|summary| summary.get("browserCapabilityLaunches"))
        .and_then(|summary| summary.get("launches"))
        .and_then(|value| value.as_array())
        .filter(|launches| !launches.is_empty())?;
    let mut lines = vec![format!("Browser capability launches: {}", launches.len())];
    lines.extend(
        launches
            .iter()
            .map(format_service_trace_browser_capability_launch_line),
    );
    Some(lines.join("\n"))
}

fn format_service_trace_browser_capability_launch_line(launch: &serde_json::Value) -> String {
    let source = launch
        .get("source")
        .and_then(|value| value.as_str())
        .unwrap_or("unknown");
    ...
    let host = launch
        ...
        .unwrap_or_default();
    format!(
        "  source={source} session={session} browser={browser} profile={profile} build={browser_build} applied={applied} reason={reason}{binding}{host}{executable}"
    )
}
        ...
        .map(|rows| {
            rows.iter()
                .filter_map(|row| {
                    ...
                    let seeding = value_str(row, "seedingMode", "not_required");
                    let cdp = value_bool_label(row, "cdpAttachmentAllowedDuringSeeding");
                    let keyring = value_str(row, "preferredKeyring", "none");
                    Some(format!(
                        "{target}:{state}:{action}:seeding={seeding}:cdp={cdp}:keyring={keyring}"
                    ))
                })
                .collect::<Vec<_>>()
                .join(",")
...
fn format_service_session_line(session: &serde_json::Value) -> String {
    ...
    let lease_disposition = value_str(session, "profileLeaseDisposition", "unknown");
    let lease_conflicts = session
        .get("profileLeaseConflictSessionIds")
        .and_then(|value| value.as_array())
        .map(|values| {
            values
                .iter()
...
fn format_service_prune_retained_text(data: &serde_json::Value) -> Option<String> {
    ...
    let orphaned_profiles = counts
        ...
        .and_then(|value| value.as_u64())
        .unwrap_or(0);
    let display_allocations = counts
    ...
    let total = counts
        .get("total")
        .and_then(|value| value.as_u64())
        .unwrap_or(closed_tabs + browsers + sessions + orphaned_profiles + display_allocations);
    ...
    let removed_orphaned_profiles = removed
        ...
        .and_then(|value| value.as_u64())
        .unwrap_or(0);
    let removed_display_allocations = removed
        .get("displayAllocations")
        .and_then(|value| value.as_u64())
        .unwrap_or(0);
    ...
    let before_tabs = before
        ...
        .and_then(|value| value.as_u64())
        .unwrap_or(0);
    let before_display_allocations = before
        .get("displayAllocationCount")
        .and_then(|value| value.as_u64())
        .unwrap_or(0);
    ...
    let after_display_allocations = after
        .get("displayAllocationCount")
        .and_then(|value| value.as_u64())
        .unwrap_or(before_display_allocations);
    let next = data
        .get("recommendedNextStep")
        .and_then(|value| value.as_str())
    ...
    let mut lines = vec![
        format!(
            "Retained service prune {mode}: candidates={total} closed_tabs={closed_tabs} browsers={browsers} sessions={sessions} orphaned_profiles={orphaned_profiles} display_allocations={display_allocations}"
        ),
        format!("Before: profiles={before_profiles} browsers={before_browsers} tabs={before_tabs} display_allocations={before_display_allocations}"),
    ];
    ...
    lines.extend([
        format!(
            "Removed: profiles={removed_orphaned_profiles} browsers={removed_browsers} closed_tabs={removed_tabs} sessions={removed_sessions} display_allocations={removed_display_allocations}"
        ),
        format!("After: profiles={after_profiles} browsers={after_browsers} tabs={after_tabs} display_allocations={after_display_allocations}"),
        format!("Next: {next}"),
    ]);
    Some(lines.join("\n"))
}
...
fn format_service_site_policy_line(
    ...
    let build = value_str(policy, "browserBuild", "default");
    ...
    format!(
        "{id} origin={origin} source={source} overrideable={overrideable} host={host} build={build} cdp_free={cdp_free} interaction={interaction} challenge={challenge} manual_login={manual_login} profile_required={profile_required}"
    )
}
...
fn format_service_monitor_line(monitor: &serde_json::Value) -> String {
    ...
    let last_succeeded = value_str(monitor, "lastSucceededAt", "never");
    let last_failed = value_str(monitor, "lastFailedAt", "never");
    ...
    format!("{id} name={name} state={state} target={target} interval_ms={interval} last_checked={last_checked} last_succeeded={last_succeeded} last_failed={last_failed} last_result={last_result} consecutive_failures={consecutive_failures}")
}
...
fn format_service_monitors_text(data: &serde_json::Value) -> Option<String> {
    ...
    let total = data
        .get("total")
        .and_then(|value| value.as_u64())
        .unwrap_or(monitors.len() as u64);
    let mut lines = vec![format!("Monitors: {} of {}", monitors.len(), total)];
    if let Some(summary) = data.get("summary") {
        lines.push(format!(
            "Summary: active={} paused={} faulted={} failing={} repeated_failures={} never_checked={} last_failed={}",
            summary.get("active").and_then(|value| value.as_u64()).unwrap_or(0),
            summary.get("paused").and_then(|value| value.as_u64()).unwrap_or(0),
            summary.get("faulted").and_then(|value| value.as_u64()).unwrap_or(0),
            summary.get("failing").and_then(|value| value.as_u64()).unwrap_or(0),
            summary
                .get("repeatedFailures")
                .and_then(|value| value.as_u64())
                .unwrap_or(0),
            summary
                .get("neverChecked")
                .and_then(|value| value.as_u64())
                .unwrap_or(0),
            value_str(summary, "lastFailedAt", "never"),
        ));
    }
...
fn format_service_status_text(data: &serde_json::Value) -> Option<String> {
    ...
    if let Some(retained_displays) = data.get("retainedDisplayAllocations") {
        let count = retained_displays
            ...
            .and_then(|value| value.as_u64())
            .unwrap_or(0);
        let apply_safe_count = retained_displays
            .get("applySafeCount")
            .and_then(|value| value.as_u64())
            .unwrap_or(0);
        let retained_count = retained_displays
            .get("retainedCount")
            .and_then(|value| value.as_u64())
            .unwrap_or(0);
        let class_counts = retained_displays
            .get("classCounts")
            .and_then(|value| value.as_object())
            .map(|counts| {
                let mut parts = counts
                    .iter()
                    .filter_map(|(class_name, count)| {
                        count.as_u64().map(|count| format!("{class_name}={count}"))
                    })
                    .collect::<Vec<_>>();
                parts.sort();
                if parts.is_empty() {
            ...
            .unwrap_or_else(|| "none".to_string());
        lines.push(format!(
            "Retained display allocations: total={count} apply_safe={apply_safe_count} retained={retained_count} classes={class_counts}"
        ));
    }
...
Global Options:
  ...
  --browser-host <host> Launch host hint for a fresh daemon
  --view-stream-provider <provider> Remote view provider metadata
  --control-input-provider <provider> Remote control-input metadata
  --display-isolation <policy> Remote display allocation policy
...
Operations:
  new [url]            Open new browser window

Options:
  --same-profile       Open the new window in the current browser profile
...
Examples:
  ...
  agent-browser window new https://example.com --same-profile
"##
        }
...
Dashboard sections are URL-addressable at /service, /browsers, and /activity.
...
left workspace navigator renders live Agent-browser owned browsers, live
needs-attention browser diagnostics, and detected non-owned addressable
browsers, while stale, retained, and resolved incident records stay in Service,
trace, event, job, incident, and log viewers. Rows show service-sourced disabled action reasons, selected-row PID,
...
the embedded workspace viewport when retained stream metadata allows it;
job-only responses fall back to Service Jobs.
The Service view includes a remembered operator identity, optional notes, an incident history timeline,
a shared events/jobs/incidents/activity trace explorer for service, agent,
task, browser, profile, session, and time-window debugging with ownership
summary cards, a browser-health transition timeline, the backend-owned
profileAllocations view with detail inspection, a grouped incident browser
panel with acknowledgement and resolution actions, human-takeover lease
visibility, and incident filters for handling state plus crash, disconnect,
...
Remote-control install readiness:
  ...
  result and remoteControl.installReady is the effective single-route gate.
  Duplicate-profile pressure is nonblocking only when it is the sole install
  issue and readiness-impacting resource candidates are zero; accepted warning
  codes remain visible in nonBlockingInstallIssueCodes and the embedded install
  report. The remote-view open request still rejects same-profile conflicts.
...
Notes:
  ...
  - Service requests with params.browserHost=remote_headed launch headed Chrome
    ...
    has service-owned route metadata. A bare Guacamole root URL is preserved as a
    root URL rather than repaired with a built-in connection hash.
    Set AGENT_BROWSER_REMOTE_VIEW_PROVIDER=rdp_gateway for an HTML5 RDP gateway
  ...
  - Linux remote_headed launches disable Chromium's Vulkan/ANGLE path and force
    ...
    URL, view provider, and control-input environment hints into an existing
    daemon instead of relying on stale daemon environment.
...
Usage:
  ...
  agent-browser service monitors triage <monitor-id> [--by <text>] [--note <text>]
...
Commands:
  ...
  monitors triage      Acknowledge related monitor incident and clear reviewed failures
...
Notes:
  ...
  - service prune-retained --orphaned-profiles removes custom:* profile records that have no retained service references and point at missing ephemeral user-data directories, plus unreferenced nonpersistent managed_one_time profile records.
  ...
  - service remedies apply --escalation browser_degraded batches retry enablement for active degraded-browser incidents after operator review.
  ...
  - Runtime profile and custom profile launches populate linked service profile and session records, including profileSelectionReason, profileLeaseDisposition, profileLeaseConflictSessionIds, and browserCapabilityLaunch diagnostics when known. When an access plan supplies both, dashboard guided launches pass the runtime-profile identity and exact custom profile path together instead of substituting a same-named managed runtime directory.
...
Notes:
  ...
  - remote_view_open returns an authenticated durable /remote-view/<handoff-id> operator URL in externalUrl and handoffUrl. Software clients should use requestServiceRemoteViewHandoff(), which returns only handoffId and handoffUrl. requestServiceRemoteViewOpen() rejects a real response without a durable handoff unless allowRawProviderUrl=true is explicitly selected for provider diagnostics. providerExternalUrl and routeBinding URLs identify only the current provider connection. Use action=service_remote_view_handoff_resolve with params.handoffId to reacquire expired route state. Authenticated recovery reattaches only the immutable retained PID, profile, exact browser WebSocket, build proof, and target; missing, changed, legacy, or ambiguous evidence fails without launching Chrome or selecting another tab. Set params.allowReopenClosed=true only after explicit operator confirmation.
  ...
  - external_byop_adopt requires browserPid when the registered profile declares browserBuild. Linux verifies the exact process, executable SHA-256 against a validated local registry row, profile, and loopback CDP listener before retaining build proof. Missing evidence rejects adoption; an unlabelled external profile remains an unproven build.
  - service_browser_repair with params.browserId and params.reverifyRetainedByop=true restores an omitted PID on a ready attached_existing browser only after the saved BYOP proof, live executable digest, process start identity, profile, endpoint, tab, and exclusive lease match. It preserves the browser and tabs; a mismatch changes nothing.
  - ui_action os_click uses viewportX and viewportY from a fresh CDP screenshot only on a verified external BYOP Linux/X11 lane. Set AGENT_BROWSER_OS_CLICK_ISOLATED_DISPLAY to that daemon's private DISPLAY and provide xdotool plus ImageMagick import. Local dashboard publication preserves that display opt-in only for the same replacement daemon session. The service matches the exact visible X11 window and fresh viewport image before one OS mouse click; ambiguous evidence fails before input. A fully scrubbed Chromium process environment may use exact X11 window and image proof instead of a readable DISPLAY; an explicit different DISPLAY still fails.
  - browser_clear and ui_action clear also support contenteditable fields. They report an error if text remains, so verify the field before sending or closing a draft.
  ...
  - Service profile records distinguish caller service sharing from target-service login scope: sharedServiceIds lists allowed caller services, targetServiceIds lists intended login targets, accountIds lists intended user or tenant identities, and authenticatedServiceIds lists targets currently believed authenticated.
  - Service profile records and profile allocation rows include targetReadiness, a no-launch readiness view for target services. Google targets without authenticated evidence report needs_manual_seeding, seedingMode=detached_headed_no_cdp, cdpAttachmentAllowedDuringSeeding=false, preferredKeyring=basic_password_store, and setup scopes for sign-in, Chrome sync, passkeys, and browser plugins. Once a managed profile lists the target in authenticatedServiceIds, readiness changes to seeded_unknown_freshness and access-plan no longer treats first-login seeding as a required manual action. Access-plan monitorFindings reports active profile_readiness monitor incidents and matching due or never-checked profile_readiness monitors for the requested target identities, and decision.monitorRunDue points clients at the serialized run-due monitor path. Explicit fresh, stale, and blocked_by_attached_devtools rows, plus rows with lastVerifiedAt or freshnessExpiresAt, are preserved through derived readiness refreshes. Remote-view allocation records are exposed through HTTP GET /api/service/display-allocations, /api/service/remote-view-routes, /api/service/route-pool, and /api/service/viewer-leases plus MCP agent-browser://display-allocations, agent-browser://remote-view-routes, agent-browser://route-pool, and agent-browser://viewer-leases so clients can compare displayAllocationId and routeId instead of parsing Guacamole URLs. Software clients can use updateServiceProfileFreshness for service-side bounded probe updates that preserve unrelated profile fields, advance matching closed seeding handoffs to verification_pending or fresh, and upsertServiceProfileReadinessMonitor for the standard retained freshness monitor.
  - Launch commands without explicit profile or runtimeProfile can include serviceName plus targetServiceId, targetService, targetServiceIds, targetServices, siteId, siteIds, loginId, loginIds, accountId, accountIds, or url so persisted service profiles are selected by authenticated target match, then account match, then target match, then caller service match.
  ...
  - Implemented resources are agent-browser://contracts, agent-browser://browser-capability-registry, agent-browser://access-plan, agent-browser://access-plan{?serviceName,agentName,taskName,targetServiceId,targetServiceIds,siteId,siteIds,loginId,loginIds,accountId,accountIds,url,sitePolicyId,challengeId,readinessProfileId,browserBuild,browserHost,viewStreamProvider,controlInputProvider,displayIsolation}, agent-browser://profiles/lookup{?serviceName,targetServiceId,targetServiceIds,siteId,siteIds,loginId,loginIds,accountId,accountIds,url,readinessProfileId,browserBuild}, agent-browser://profiles/{profile_id}/readiness, agent-browser://profiles/{profile_id}/allocation, agent-browser://profiles/{profile_id}/seeding-handoff{?targetServiceId,siteId,loginId}, agent-browser://display-allocations, agent-browser://remote-view-routes, agent-browser://route-pool, agent-browser://viewer-leases, agent-browser://incidents, agent-browser://profiles, agent-browser://sessions, agent-browser://browsers, agent-browser://tabs, agent-browser://monitors, agent-browser://site-policies, agent-browser://providers, agent-browser://challenges, agent-browser://jobs, agent-browser://events, and agent-browser://incidents/{incident_id}/activity.
...
Options:
  ...
  --args <args>              Browser launch args, comma or newline separated (or AGENT_BROWSER_ARGS)
                             Applied only to commands that can launch a browser
                             e.g., --args "--no-sandbox,--disable-gpu"
  ...
  --browser-build <build>    Browser build hint: stock_chrome, stealthcdp_chromium, cdp_free_headed
  --browser-host <host>      Launch host hint: local_headless, local_headed, docker_headed, remote_headed, cloud_provider, attached_existing
  --view-stream-provider <provider> Remote view metadata: cdp_screencast, chrome_tab_webrtc, virtual_display_webrtc, novnc, rdp_gateway, external_url
  --control-input-provider <provider> Remote input metadata: cdp_input, webrtc_input, vnc_input, manual_attached_desktop
  --display-isolation <policy> Remote display policy: private_virtual_display, shared_display, ambient_display
...
Configuration:
  ...
  Pass `--leave-open` on `close` itself for a one-time detach of an already
  running managed browser; its live runtime identity must match the record.
...
mod tests {
    ...
    fn test_format_service_status_text_includes_profile_and_session_summaries() {
        let data = json!({
            ...
            "retainedDisplayAllocations": {
                "count": 3,
                "applySafeCount": 2,
                "retainedCount": 1,
                "classCounts": {
                    "safe-orphan-display": 1,
                    "stale-route-reference": 1,
                    "diagnostic-retained": 1
                },
                "applySafeIds": ["display-orphan", "display-stale"],
                "retainedIds": ["display-diagnostic"],
                "candidateReasons": {},
    ...
    fn test_format_service_prune_retained_text_includes_counts_and_next_step() {
        let data = json!({
            ...
            "removed": {
                ...
                "sessions": 0,
                "orphanedProfiles": 0,
                "displayAllocations": 0
            },
            "before": {
                "profileCount": 20,
                "browserCount": 10,
                "tabCount": 30,
                "displayAllocationCount": 8
            },
            "after": {
                "profileCount": 20,
                "browserCount": 10,
        ...
        assert!(rendered.contains(
            "Retained service prune dry-run: candidates=15 closed_tabs=3 browsers=2 sessions=1 orphaned_profiles=4 display_allocations=5"
        ));
        assert!(rendered.contains("Before: profiles=20 browsers=10 tabs=30 display_allocations=8"));
        ...
        assert!(rendered.contains(
            "Removed: profiles=0 browsers=0 closed_tabs=0 sessions=0 display_allocations=0"
        ));
        assert!(rendered.contains("After: profiles=20 browsers=10 tabs=30 display_allocations=8"));

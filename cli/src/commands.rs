...
mod tests {
    ...
    fn test_service_monitors_pause_resume_reset_and_triage() {
        ...
        let resume = parse_command(
            &args("service monitors resume google-login-freshness"),
            &default_flags(),
        )
        .unwrap();
    ...
    #[test]
    fn test_service_monitors_pause_requires_monitor_id() {
        let err = parse_command(&args("service monitors pause"), &default_flags()).unwrap_err();
    ...
    #[test]
    fn test_service_remedies_apply_os_degraded_possible() {
        let cmd = parse_command(
            &args("service remedies apply --escalation os_degraded_possible --by operator"),
            &default_flags(),
        )
        .unwrap();

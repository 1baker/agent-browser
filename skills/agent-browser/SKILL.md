...
delete metadata or replay a failed mutation to force the cold path. This is a
profile-configuration exception, not retained-browser acquisition authority.

For ordinary daemon-crash recovery, first use the original session's `tab list`
without new launch flags. Implicit tab-list, browser-PID and CDP-endpoint reads
enter daemon acquisition directly. This is not a generic no-launch guarantee:
missing retained candidates follow existing acquisition policy. Verify original
PID, endpoint and target inventory before reading page content or resuming work.

Repeated launch options recheck owned installed-Chrome evidence before saving
reuse metadata. For disposable stock-Chrome configuration, use
`service.defaultBrowserBuild: "stock_chrome"`, not top-level `browserBuild`.
An unprepared daemon crash is not a prepared handoff: test same-session read
reconnection and verify the exact retained target and effect count. Never infer
crash recovery from a missing handoff-descriptor error or read success alone.

MCP handle-only requests route using the original handle's `sessionName` or
`session:` browser ID. Conflicting selectors and unroutable handles are rejected
before transport. A missing retained daemon must not fall back to the default
session or launch a replacement. Routing does not refresh stale handles or grant
profile or target authority. Older installed versions require the access plan's
matching explicit top-level `browserId` and `sessionName` as well as the handle.

When the jobs resource exceeds tool-output limits, use the checkout helper
`node scripts/read-service-job-summary.js /absolute/path/to/agent-browser`.
It captures the complete no-launch resource locally with bounded time and bytes,
then emits only counts. Exit 0 means observed idle, 2 means nonterminal work,
and 1 means unavailable. Unknown states block idle. Never treat this snapshot
as a lease, browser authority, or rendered-page verification.

For reviewed legacy publisher maintenance, use `--prebuilt-bin <absolute-path>`
with `--expected-sha256 <sha256>`, `--expected-sessions <comma-separated-names|none>`,
`--skip-reference-sync`, and `--skip-browser`. This snapshots reviewed embedded
dashboard bytes without rebuilding; HTTP and manifest checks remain required.
The publisher refuses generation-selector symlinks and changed session sets.
These checks do not fence independent clients; arrange a quiet maintenance window.
To update an existing controller script with a matching reviewed binary, use
`--controller-update scripts/name.js=/absolute/reviewed/name.js=sha256` with
prebuilt mode. It verifies embedded bytes and snapshots both controller versions
alongside binary/manifest provenance. Recovery uses those snapshots, not mutable
source paths. Verify isolated installer extraction and rollback first; preserve
unlisted scripts. Rearm the separate Workshop helper after installed verification.
It requires the exact installed runtime-interlock timer/service and lock contract,
stops only the timer, and refuses a running oneshot. Never kill the interlock.
Native reconcile units are supported alongside legacy flock units. Native custody
is a non-PID marker that survives publisher death; only exact receipt-bound file
recovery may remove it. Foreign locks and unproven workstation roots are refused.
Uncertain handoff failures retain stopped-timer custody in the journal's adjacent
`.interlock.json` receipt. Use its exact ID with `--recover-only
--recover-interlock-receipt <id>` only after the prior publisher ended. Missing
complete handoff evidence remains blocked. Do not erase receipts, rearm the timer,
or infer newer generation-upgrade guarantees from this legacy publisher.
Same-version workstation publication preserves verified support files and stores
both installation-record snapshots in the publication journal before replacing
the binary. Recovery selects the record matching the verified installed binary;
rollback restores the original record. Do not manually edit manifest hashes.
Publication checks its own exact journal/lock warning in scope, then requires a
strict installed doctor after releasing custody. Dashboard quiescence may retire
only the proven idle, browser-free service backend; unrelated session/PID changes
refuse handoff. A source rollback with hash-verified deleted-inode controllers is
reported degraded, not installation-ready; the next guarded publication repairs
those controllers. Never ignore unrelated doctor issues.
...
session.
Healthy existing installations report `healthy-runtime-preserved` after fresh
provenance, retained identity, unit, and canonical-route checks, without service
restarts or browser/configuration changes. Unknown evidence and active route
conflicts fail closed. Guacamole authentication probes may issue temporary
tokens. Fresh apply still bootstraps; the healthy check does not replace it.
...
requires that policy only when the kernel enables AppArmor and restricts
unprivileged user namespaces; an AppArmor-disabled WSL kernel reports it as not
required. It keeps Chromium's sandbox enabled instead of adding `--no-sandbox`
or disabling the host restriction. The doctor recognizes the pinned Compose Guacd
...
Run `agent-browser install workstation retained-browser-status --json` to
audit that same native authority without a workstation lock, apply, reconcile,
daemon call, browser launch, or state creation. It honors
`AGENT_BROWSER_DASHBOARD_RETAINED_REQUIREMENT` from `AGENT_BROWSER_ENV_FILE` or
the normal `~/.agent-browser/.env`, so the installed timer and an interactive
audit resolve the same configured path without manual flags or exports.

Pinning commits a separate private `.required` enforcement record before the
requirement. A crash between commits is safely retryable. Once enforcement
exists, a missing requirement makes native status, apply, reconcile, and
repository publication fail closed instead of reverting to `not_configured`.
The marker precommits the exact requirement SHA-256; crash retry reuses it, and
a stale marker paired with a replaced requirement fails before service-state
access.

After an operator has selected the intended page in a retained browser, use
`pnpm pin:local-dashboard-retained-browser --
--discover-retained-url-prefix <reviewed-origin-and-path-prefix>` to derive the
stable session, profile, target, and exact canonical URL without copying four
identity fields. Discovery is read-only until the existing marker-first commit.
It requires exactly one live `health=ready` page target beneath the reviewed
prefix and fails closed on zero or multiple matches, unreadable live lanes,
incomplete PID/profile identity, degraded health, or origin/path drift. It does
...
```bash
agent-browser service access-plan \
...
For slow live Guacamole/RDP fixture validation, set
`AGENT_BROWSER_REMOTE_VIEW_OPEN_TIMEOUT_MS` before
...
Responses also include `browserBuildProof` with requested build, selected
build, executable path when known, and mismatch state.
Likely one-time operator handoffs that omit `runtimeProfile` use a deterministic
`managed_one_time` task profile so retries reuse the same lane. If the caller
passes an unknown runtime profile instead, responses include
`oneTimeProfileWarning` with the requested profile as `operator_supplied` and a
...
pool or access-plan response. `--provider rdp_gateway` remains a
...
`getServiceRemoteViewRoutePreflight()` when a client needs a no-launch route
eligibility check before opening. Its response includes `fastPreflight.status`
(`ready`, `partial`, `stale`, or `blocked`) plus component evidence for
...
state.
Use `service_remote_view_browser_reattach` to show a retained RDP browser again
without launching Chrome or acquiring the runtime profile. Use
`service_remote_view_route_switch` when the caller has a specific browser and
needs the service to bind it to a different available or parkable Guacamole
route. When no route-pool entry is available, route switch may park another
live browser's route, release that route surface, and return
...
readiness smoke or doctor exports route-pool data so the daemon plans against
the current route records instead of stale retained route-pool state.
Run `pnpm test:remote-view-route-preflight-timing` when validating the isolated
no-launch HTTP/client route-preflight path and bounded fast-preflight latency.

If a bounded direct CLI launch is appropriate and the route-bound open path is
not the right fit, first verify that `agent-browser service status` reports
`launchConfig.defaultBrowserBuild` as `stealthcdp_chromium` and
`launchConfig.stealthCdpChromiumReady` as ready, then preserve the
remote-control posture explicitly:
...
Prefer `remote-view open`, the dashboard, or a queued service request when
available because they record the Guacamole view URL, display, input provider,
profile lease, session, and browser health as service state. Use local headless
or direct local headed sessions only for throwaway no-login smokes, non-visual
extraction, or when an operator explicitly asks for a local browser. Use
...
`navigation`, `page_mutation`, `external_mutation`, `file_transfer`,
`credentials`, `script_execution`, `browser_lifecycle`, and `control_plane`;
`mutation` selects the combined mutation-sensitive group. A confirmation
receipt includes the exact ID, consequence class, active target ID and URL, and
60-second expiry. Confirm or deny with the exact ID. Confirmation fails closed
when the ID is wrong, the receipt expired, or the active target changed, and a
second request cannot replace a live pending approval. Prefer
`external_mutation,page_mutation,file_transfer,credentials,script_execution`
before authenticated or consequential browsing.

For multi-command agentic work, obtain one immutable `taskAuthority` through
HTTP `POST /api/service/task-authorities/issue` or MCP
`service_task_authority_issue`. Supply exact caller labels, retained target ID
and URL, issuer and approval reference, short lifetime, and explicit bounded
plan steps. The broker derives the smallest unique action/origin sets and exact
action/evidence budgets; issuance always requires fresh target-bound
confirmation. Inspect the durable record with the no-launch status HTTP routes
or `service_task_authority_status`, and revoke through the confirmation-gated
HTTP route or `service_task_authority_revoke`. Carry the returned envelope on
each browser command. Required mode rejects fabricated, changed, revoked,
expired, wrong-target, wrong-origin, unplanned, and over-budget authority.
Daemon restart does not reset usage.

When status reports exactly one indeterminate step, use HTTP
`POST /api/service/task-authorities/<id>/reconcile` or MCP
`service_task_authority_reconcile` with a stable `reconciliationId`, that exact
`unresolvedStepId`, fresh target ID and URL, issuer/approval evidence, and an
explicit replacement plan. The operation requires fresh target-bound
confirmation, durably revokes the predecessor before creating the replacement,
and binds predecessor authority, step, command, and indeterminate state into
the replacement envelope. An identical retry resumes the same replacement;
changed evidence fails closed. Never replay the consumed predecessor step.

The Service dashboard Authorities workspace reads the selected session's
no-launch authority collection and displays outcome counts, pending/completed
reconciliation, and replacement lineage. Recovery is enabled only for exactly
one indeterminate receipt and one exact live target. Review the explicit
replacement plan, request confirmation, then use HTTP
`POST /api/service/task-authorities/confirmation` or MCP
`service_task_authority_confirmation` with the same `sessionName`, returned
`confirmationId`, exact `expectedAction`, `decision` of `confirm` or `deny`,
and explicit `decidedBy` operator or service identity. Pending issue,
reconcile, and revoke controls survive daemon restart with private
session/action/target/URL/request-digest/expiry evidence. The decision receipt
is committed before dispatch and is single use. A crash after that commit is
reported as indeterminate and is never automatically confirmed, replayed, or
restaged. The expected action prevents this surface from approving an
unrelated pending control. Closing a staged dashboard recovery denies it.
Never treat staging as approval.
...
Before asking an operator to authenticate through Guacamole/RDP, run
`agent-browser doctor remote-view --json` and require `remoteControl.status` to
be `ready`. For multi-viewer claims, also require `manyToMany.status` to be
`ready`. A terminal-only display, missing browser-window proof, route display
...
checked-out routes before launch planning. Agent-browser does not establish an
...
existing-user sync spelling is a compatibility alias to the route-specific
migration because same-user color-depth isolation is not supported.

When validating dashboard, Guacamole, or `rdp_gateway` remote-control UX, use two independent browser clients before calling the path healthy. Use different browser executables when available, for example Google Chrome and Brave, with separate `--session` names and separate `--profile` directories. Do not count a single warmed local client as evidence for an external operator path.

The validation must prove one of these outcomes with screenshots or DOM evidence:

1. Both clients can view the same remote desktop at the same time.
2. If the gateway is single-viewer, disconnect or unload one client and prove the other client can take over and render the remote browser.
...
For sites that aggressively challenge automation, start with the preferred `chromium-stealthrdp` posture above when CDP-backed control remains acceptable. It gives agents a headed, remotely inspectable, patched Chromium path without taking over the operator's local desktop.

Fall back to direct local Chrome only when the operator specifically needs a local Chrome profile or no remote display is configured:
...
Direct local headed Chrome is still materially more reliable than a fresh headless session for sites like Google, Gmail, and other consumer properties that react to automation signals, but it is no longer the default agent posture when the hidden remote-headed path is available.

## Handling Authentication

### Remote operator authentication handoff

When an agent-controlled browser reaches sign-in, MFA, passkey, CAPTCHA, consent,
or another operator-only checkpoint, keep the exact retained browser and profile
as the authority. Do not open the checkpoint on the operator's ambient home
desktop, copy cookies or session state into Slack, start a second browser on the
same profile, or ask the operator to send a password or verification code in
chat.

For an operator who is away from the workstation:

1. Run `agent-browser doctor remote-view --json` and require
   `remoteControl.status=ready` before promising phone control.
2. Use `remote-view open` with the exact runtime profile and target URL. Require
   `operatorVisible.state=ready` and use only the returned durable `handoffUrl`.
   Never notify with a raw Guacamole connection URL, local dashboard URL, CDP
   endpoint, profile path, target id, document URL with query parameters, or
   authentication material.
3. When the Slack connector is available, resolve the current Slack user, reuse
   or create that user's private self-DM, and send a compact action-required
   message containing the site name, a nonsensitive task label, the durable
   handoff URL, and the instruction to reply `done` after finishing. Do not post
   remote-authentication requests in public, shared, or group channels unless
   the operator explicitly selected that destination.
4. Continue the original browser job only after a bounded post-authentication
   probe succeeds or the operator replies `done` and a fresh probe succeeds.
   A Slack reply by itself is not proof that authentication completed.
5. If remote-view readiness is blocked, send or report the specific
   infrastructure blocker without a browser link. Preserve the retained browser
   and wait for the route to become ready; do not fall back to the ambient home
   display for an away operator.

Slack is the notification and acknowledgement channel. The authenticated
remote-view page is the only place where the operator enters credentials,
approves MFA, completes Duo, or interacts with protected document content.

For a private Tailscale-only phone route to the localhost dashboard, enable
`AGENT_BROWSER_DASHBOARD_TAILSCALE_AUTH=1` and set
`AGENT_BROWSER_DASHBOARD_TAILSCALE_ALLOWED_LOGINS` to the exact operator login.
The dashboard accepts that identity only for HTTPS `.ts.net` requests carrying
the matching Tailscale Serve header. Never enable this trust mode on a listener
also reached by Funnel, Cloudflare, or another reverse proxy. Keep password
login or Authelia for those routes.

The authenticated dashboard origin proxies `/guacamole/*` to the loopback
Guacamole service, strips caller identity headers and dashboard cookies, and
injects `AGENT_BROWSER_GUACAMOLE_HEADER_USER` only after dashboard
authentication. For a private tailnet handoff, set
`AGENT_BROWSER_REMOTE_VIEW_EXTERNAL_URL` to
`https://<node>.<tailnet>.ts.net/guacamole/`. Keep the local frame URL for
workstation readiness, use only the resulting durable
`/remote-view/<handoff-id>` URL for phone or Slack handoff, and never publish
the raw Guacamole client URL or port 8092.
...
`loginId`, `siteId`, `targetServiceId`, `accountId`, or `url` before registering a profile. Then
request the tab by the same identity through `requestServiceTab()` or
`POST /api/service/request`. A successful tab request can return a
`serviceTabHandle`; use `requireServiceTabHandle()` before follow-on work and
`requestServiceCdpAttach()` only when the access plan allows CDP attachment.
Use `requestServiceCdpDetach()` when the client is done so the detach is traced
without closing the browser process by default. Use `requestServiceEvaluate()`
for bounded JavaScript reads against the same valid handle, and always set
positive `timeoutMs` and `maxReturnBytes`. Use `requestServiceDiagnostics()`
to collect compact URL/title, browser, session, tab, profile-readiness,
route/view, console, error, request-summary, snapshot-summary, caller, trace,
and optional screenshot evidence for the same handle. Use
`getServiceProfiles()` or
...
Add `--account-id <id>` or `--account-ids <id,id>` when the operator has a
safe account label to associate with the verified profile.
...
For service-owned workflows, prefer generic probe requests over direct raw CDP
when the goal is identity, account, readiness, or compact page-state evidence.
Request a leased service tab, keep the returned `serviceTabHandle`, then use
`action: "probe"` with `timeoutMs`, `maxReturnBytes`, and a provider-neutral
`probe.detectors` recipe. Built-in detector types are `url_title`,
`selector_text`, `evaluate`, and `client_evidence`; clients can provide
site-specific selectors or evaluation snippets inside the recipe. Include
`probe.recordFreshness` only when the probe should merge target and account
freshness evidence into a service profile.

For repeated service-owned UI steps, prefer `runServiceUiAction()` or service
request `action: "ui_action"` over raw CDP `Input`, `DOM`, or small `Runtime`
...
For service-owned network evidence, prefer `captureServiceNetwork()` or service
request `action: "network_capture"` over raw CDP `Network` subscriptions and
`Network.getResponseBody`. The request must use a valid `serviceTabHandle`, a
positive timeout, and a `networkCapture` recipe with positive `maxEvents`.
Metadata is returned by default with headers redacted. Set
`captureBodies: true` only when body capture is needed, and always provide a
positive `maxBodyBytes`; the daemon caps body output and returns truncation
metadata. Keep URL, method, resource type, and status filters provider-neutral.

For service-owned upload and download work, prefer `transferServiceFiles()` or
service request `action: "file_transfer"` over raw `DOM.setFileInputFiles`,
`Browser.setDownloadBehavior`, or `Page.setDownloadBehavior`. The request must
use a valid `serviceTabHandle`, a positive timeout, and a `fileTransfer` recipe
with `upload` and/or `download`. Upload recipes require a selector or visible
`labelText`, explicit `files`, `allowedPaths`, and `maxFiles`. Download recipes
require a click selector, `directory`, `allowedDirectories`, and optional
expected file name or `maxBytes`. Treat the expected name as a validation
constraint; a different provider-suggested name must fail rather than be renamed.
Require SHA-256 from upload and download receipts before accepting transferred
proposal evidence or deliverables. Keep website-specific selectors and labels in
the caller-owned recipe; agent-browser owns path allowlist checks, caps, trace
linkage, selected file-name verification, provenance metadata, and optional
...
uses the policy-gated attach descriptor, runs a generic identity or account
probe, runs a caller-owned UI action recipe, captures capped network evidence,
performs allowlisted file transfer, collects diagnostics, and detaches. Keep
website-specific detection, selectors, expected text, and extraction
instructions in the caller-provided recipes. Validate the full path with
`pnpm test:service-composed-workflow-live` when changing the composed example or
...
Metadata-preserving Linux handoff rechecks exact retained build evidence in the
same health transaction before deriving attachability. Failed process proof
clears stale build/path fields, not profile, tab or display custody. This does
not authorize handoff, installation, credential entry or renewal by itself.
...
agent-browser loads `AGENT_BROWSER_ENV_FILE` first when set, otherwise `~/.agent-browser/.env` if it exists. Environment variables still override file values. On macOS, `AGENT_BROWSER_KEYCHAIN_PASSWORD` unlocks the login keychain before Chrome launches. On Linux, it is used with `gnome-keyring-daemon --unlock --components=secrets`, which is useful on Ubuntu and other GNOME-keyring setups. The same dotenv loader also accepts `AGENT_BROWSER_REMOTE_HEADED_EXECUTABLE_PATH`, `AGENT_BROWSER_REMOTE_HEADED_DISPLAY`, `AGENT_BROWSER_REMOTE_VIEW_URL`, `AGENT_BROWSER_REMOTE_VIEW_FRAME_URL`, `AGENT_BROWSER_REMOTE_VIEW_EXTERNAL_URL`, `AGENT_BROWSER_REMOTE_VIEW_ROUTE_ID`, `AGENT_BROWSER_GUACAMOLE_CONNECTION_ID`, `AGENT_BROWSER_GUACAMOLE_CONNECTION_NAME`, `AGENT_BROWSER_GUACAMOLE_HEADER_USER`, `AGENT_BROWSER_GUACAMOLE_HTTP_PORT`, `AGENT_BROWSER_REMOTE_VIEW_PROVIDER`, `AGENT_BROWSER_REMOTE_CONTROL_INPUT_PROVIDER`, `AGENT_BROWSER_RDP_ROUTE_VIEWER_EXECUTABLE`, `AGENT_BROWSER_DASHBOARD_TAILSCALE_AUTH`, `AGENT_BROWSER_DASHBOARD_TAILSCALE_ALLOWED_LOGINS`, and `AGENT_BROWSER_DASHBOARD_AUTH_FILE` for user-scoped dashboard and remote-headed setup.
...
agent-browser window new https://example.com --same-profile  # Open a same-profile window
...
Use `get page` when a large or renderer-stalled document needs bounded evidence
extraction. It uses Chromium network and IO commands in an ephemeral background
target, closes that target before returning, does not use `Runtime.evaluate` or
navigate the active target, honors the domain allowlist, and excludes browser
credentials unless `--include-credentials` is explicit. The hard limits are
1,000,000 bytes and 60 seconds.
...
hash mismatch or other readiness failures remain fatal.
...
Streaming is opt-in. Use `agent-browser stream enable` to start a runtime WebSocket stream server, then `agent-browser stream status` to inspect the bound port and connection state. Use `stream disable` to tear it down, and `stream enable --port <port>` to bind a specific port. Service-owned local headless, local headed, and attached CDP browsers can expose a `cdp_screencast` `viewStreams` record backed by the session stream server. Ready records use `controlInput="cdp_input"` and loopback `url` or `frameUrl`; unavailable records keep `readOnly=true`, omit openable URLs, and put the reason in `readiness.reason`.
...
Use `agent-browser service access-plan` when a shell operator or CLI-driven agent needs the same no-launch broker recommendation as HTTP `GET /api/service/access-plan`, MCP `service_access_plan`, or `agent-browser://access-plan`. Pass `--service-name`, `--agent-name`, `--task-name`, `--login-id`, `--target-service-id`, `--account-id`, `--url`, `--browser-build`, `--browser-host`, `--view-stream-provider`, `--control-input-provider`, or `--display-isolation` when known. Text output includes selected profile, manual-seeding posture, monitor freshness recommendation, service request availability, and compact `browser_build_summary` fields so the CLI path has the same routing audit information as software clients. JSON responses include `decision.profileReuse`, which tells clients and dashboard launcher eligibility rows whether to reuse an existing compatible browser, wait for the selected profile lease, or launch a new browser because no compatible account, site, build, host, stream, input, and display lane exists. When the recommendation is `reuse_existing_browser`, copy the planned `decision.serviceRequest.request` with its top-level `browserId` and `sessionName` route hints so `POST /api/service/request`, MCP `service_request`, and generated client helpers use the existing daemon lane. Direct launches that select a profile already backed by a live retained browser are rejected unless the request uses those route hints or sets `allowDuplicateProfileLane: true` for reviewed isolation or throwaway browser behavior.
...
Run `pnpm test:service-contracts-no-launch` to validate that HTTP `GET /api/service/contracts` returns compatibility metadata without launching or recording a browser, and that `getServiceContracts()` exposes browser capability registry, profile lookup, and readiness client-helper metadata to software clients.
...
Use `summarizeServiceAccessPlanBrowserBuildSelection()` when software needs the access-plan browser-build explanation as compact routing audit fields. It returns the selected build, source, evidence source, explicit operator override status, selected preference binding, profile compatibility status, validation evidence status, attention flags, and compact string without requiring clients to copy nested `decision.launchPosture.browserBuildSelection` parsing.

When an access plan includes both a service profile identity and an exact custom user-data path, dashboard launcher session arguments pass both `--runtime-profile <id>` and `--profile <path>` so service identity and exact stored browser state remain authoritative together.

Access-plan `decision.launchPosture` also reports `viewStreamProvider`, `viewStreamProviderSource`, `controlInputProvider`, `controlInputProviderSource`, and remote-headed `displayIsolation`. Copied queued service requests carry `params.viewStreamProvider`, `params.controlInputProvider`, and `params.displayIsolation` so agents, software clients, and the dashboard use the service-owned viewing, input, and display-allocation posture rather than inferring RDP, noVNC, WebRTC, CDP screencast, manual desktop input, or display ownership locally. Raw HTTP, MCP, and client access-plan requests can set `browserHost`, `viewStreamProvider`, `controlInputProvider`, and `displayIsolation`; raw service requests can set `displayIsolation` or `params.displayIsolation` to `private_virtual_display`, `shared_display`, or `ambient_display`. Remote-headed browser records persist the chosen input provider on each `viewStreams` entry as `controlInput`, so operator surfaces can distinguish view-only streams from controllable remote desktop streams. Non-remote CDP browser records can persist `provider="cdp_screencast"` with `controlInput="cdp_input"` when the session stream server and CDP endpoint are ready, or unavailable readiness when they are not. Service status can synthesize that CDP stream metadata from retained browser CDP endpoints plus live session stream files, and adds transient process stats for PID, memory, and CPU time when the local OS exposes them. They also expose `displayIsolation` and `displayName` when the service can tell whether the browser uses a private virtual display, an explicitly shared display, or the daemon's ambient `DISPLAY`. Dashboard workspace rows carry `WorkspaceInventoryClass` values such as `service-owned-controllable-browser`, `service-owned-diagnostic-browser`, `detected-non-owned-browser`, `viewer-client`, `retained-history`, `service-owned-session`, and `service-profile-action`, so inspector, chat, console, and automation surfaces do not infer ownership from URL shape. The dashboard enables View only for embeddable streams and Control only when service metadata includes an input provider; RDP gateway streams must also have current operator-visible browser-window proof before View, Control, or external open are enabled. Terminal-only, idle-display, missing-proof, wrong-tab, unavailable-route, missing-CDP-target, and stale-route RDP rows move to needs-attention with disabled View and Control actions instead of staying in the live control group. Workspace navigator row selection opens the embedded viewport only when a stream is available and operator-visible, and shows selected-row detail with PID, memory, CPU, CDP port, stream port, route proof state, inventory class, and action reasons.

For headed access plans, the copied service request includes `params.headless=false` and `params.browserHost`. A copied `remote_headed` request is executable in the daemon: on Linux it starts a hidden Xvfb-backed headed browser by default, keeps CDP control available, and records a view stream entry for operator or dashboard surfaces. WSL workstations whose normal manifest selects a Windows browser can set `AGENT_BROWSER_REMOTE_HEADED_EXECUTABLE_PATH` to a Linux-local Chrome for XRDP/X11 launches without changing ordinary local or broker-owned browser selection; explicit executable choices still win. Prefer `agent-browser remote-view open <url> --runtime-profile <id> --browser-build stealthcdp_chromium --view-stream-provider rdp_gateway` or service request action `remote_view_open` when a caller needs a route-bound browser, because that routine selects the route binding first, launches on the bound display, opens the tab, checks visible browser-window evidence, checks out the route, and returns dashboard and external URLs without manual route, display, or CDP repair. If post-launch setup or route verification fails, a newly launched browser is closed before the typed error is returned; reused retained browsers keep the browser process and clean up only the opened tab when possible. Use `--dry-run` to inspect the planned launch, tab, and checkout commands without launching. Linux `remote_headed` headed launches disable Chromium's Vulkan/ANGLE path and force desktop GL so XRDP and Xvfb view streams paint the browser window instead of a blank white surface. Remote-headed launches also start maximized by default and run the same native maximize step as `view_focus`, so the browser window follows Guacamole or XRDP viewport resizing when the window manager supports it. Fresh CLI launches can pass `--browser-build stealthcdp_chromium`, `--browser-host remote_headed`, `--view-stream-provider rdp_gateway`, `--control-input-provider manual_attached_desktop`, and `--display-isolation private_virtual_display` to record the same isolated stream metadata without routing through an already-running backend browser. Plain `open`, `goto`, and `navigate` preserve those explicit global routing flags, and compatible retained same-profile browsers with CDP endpoints are acquired as fresh-tab targets instead of duplicate Chrome process launches. If a daemon is already running, the prelaunch command carries current `AGENT_BROWSER_REMOTE_HEADED_EXECUTABLE_PATH`, `AGENT_BROWSER_REMOTE_HEADED_DISPLAY`, `AGENT_BROWSER_REMOTE_VIEW_URL`, `AGENT_BROWSER_REMOTE_VIEW_FRAME_URL`, `AGENT_BROWSER_REMOTE_VIEW_EXTERNAL_URL`, `AGENT_BROWSER_REMOTE_VIEW_ROUTE_ID`, `AGENT_BROWSER_GUACAMOLE_CONNECTION_ID`, `AGENT_BROWSER_GUACAMOLE_CONNECTION_NAME`, `AGENT_BROWSER_REMOTE_VIEW_PROVIDER`, and `AGENT_BROWSER_REMOTE_CONTROL_INPUT_PROVIDER` values into the daemon so stale daemon environment does not drop the Guac contract. Treat one private virtual display per browser process as the default isolation boundary, and expect access plans to express that as `params.displayIsolation="private_virtual_display"`. That keeps focus, window geometry, input injection, streaming, recording, crash recovery, and repair scoped to the selected browser. Set `displayIsolation="shared_display"` with `remoteHeadedDisplay`, `display`, or `AGENT_BROWSER_REMOTE_HEADED_DISPLAY` when an operator intentionally wants to reuse an existing shared display for trusted low-contention work. Set `displayIsolation="ambient_display"` when the daemon should deliberately use its inherited `DISPLAY`. Set `AGENT_BROWSER_REMOTE_VIEW_URL` to record an external view URL, and set `AGENT_BROWSER_REMOTE_VIEW_PROVIDER` when the view provider is not the default `cdp_screencast`. Use `AGENT_BROWSER_REMOTE_VIEW_FRAME_URL` for an embeddable Guacamole or RDP gateway route, `AGENT_BROWSER_REMOTE_VIEW_EXTERNAL_URL` for popout windows, `AGENT_BROWSER_REMOTE_VIEW_ROUTE_ID` for the service route identity, and `AGENT_BROWSER_GUACAMOLE_CONNECTION_ID` plus `AGENT_BROWSER_GUACAMOLE_CONNECTION_NAME` when the provider exposes a concrete Guacamole connection. A bare Guacamole root URL is preserved as a root URL; dashboard code does not invent a client route from a built-in hash. Use `AGENT_BROWSER_REMOTE_VIEW_PROVIDER=rdp_gateway` when that URL points to an HTML5 RDP gateway the dashboard should embed. The dashboard guided launcher defaults to `shared_display`, `rdp_gateway`, and `manual_attached_desktop`; selecting RDP gateway starts the selected browser/profile combo as a fresh `remote_headed` daemon session so the launched browser appears in the same remote desktop that Guacamole embeds. The Service inspector shows selected-browser view readiness, control readiness, display isolation, provider, input, and gateway URL from the retained stream metadata. Its selected-browser `Open remote control` action uses the same queued `view_focus` path as browser rows before opening the embedded stream. Prefer `view_focus` with `params.targetId` plus `params.index` when both are known so stale retained target IDs can fall back to tab index; use `params.index` alone only when no target ID is present. Use `view_takeover` with `params.browserId`, `params.sessionName`, `params.streamId`, `params.provider`, and `params.openMode` when an RDP or Guacamole viewer should take over or reconnect while preserving the browser process and session. Successful requests return `status`, `providerMode`, `viewerLeaseId`, `lastViewerEvent`, and `serviceEventId`, and retain a `viewer_takeover_requested` service event. The dashboard queues `view_takeover` for both embedded Take over and external open paths before refreshing or opening the stream. Run `pnpm test:rdp-gateway-readiness-live` on an operator workstation to verify local `guacd`, `xrdp`, `xfreerdp`, and TCP readiness before expecting an RDP gateway URL to work. Run `pnpm test:service-remote-view-control-live` when changing remote-headed launch, retained view-stream records, or dashboard View and Control gating; it launches an isolated remote-headed browser, verifies stream and input metadata, runs queued `view_focus`, and closes the browser through the service-owned path. Run `pnpm test:service-dashboard-remote-control-ui-live` when changing the selected-browser inspector or embedded dialog; it drives the real dashboard UI, clicks the right-pane remote-control action, verifies the iframe URL, and confirms the dashboard queued `view_focus`. Run `pnpm test:rdp-guac-viewer-transfer-live` for the two-client Slice B live gate. It requires `AGENT_BROWSER_REMOTE_VIEW_URL`, `AGENT_BROWSER_REMOTE_HEADED_EXECUTABLE_PATH`, `AGENT_BROWSER_REMOTE_HEADED_DISPLAY`, `AGENT_BROWSER_RDP_TEST_CLIENT_A_EXECUTABLE`, and `AGENT_BROWSER_RDP_TEST_CLIENT_B_EXECUTABLE`; optional overrides include `AGENT_BROWSER_RDP_TEST_BROWSER_A`, `AGENT_BROWSER_RDP_TEST_PROFILE_A`, `AGENT_BROWSER_RDP_TEST_PROFILE_B`, `AGENT_BROWSER_RDP_TEST_PUBLIC_URL`, and `AGENT_BROWSER_RDP_TEST_DISPLAY_ISOLATION`. The smoke captures service-state JSON plus desktop and mobile-size screenshots under `/tmp/agent-browser-rdp-guac-hardening-<timestamp>/`, checks simultaneous view or deterministic takeover behavior, exercises external-open `view_takeover`, and refreshes both clients before reporting success. Run `pnpm test:rdp-guac-browser-switch-live` for the Slice C managed-browser switching gate. It uses the same required inputs as the viewer-transfer smoke and also accepts `AGENT_BROWSER_RDP_TEST_BROWSER_B`. The smoke launches two `remote_headed` daemon sessions with distinct runtime profiles in one isolated service home, switches client 1 from browser A to browser B through workspace routes, refreshes B, opens browser A in client 2 while client 1 remains routed to B, alternates A to B to A to B three times, exercises external-open `view_takeover`, and captures screenshots plus service-state JSON under `/tmp/agent-browser-rdp-guac-browser-switch-<timestamp>`. Run `pnpm test:rdp-guac-readiness-failures-live` for the Slice D readiness gate. It runs the live RDP gateway readiness smoke, opens one real RDP-backed workspace for a healthy viewport screenshot, then renders isolated or fixture-backed auth, Guacamole connection, provider or ingress, viewer ownership, browser unavailable, missing stream, and stale retained-job recovery states without stopping shared provider services. Run `pnpm test:rdp-guac-route-cleanup-live` for the Slice G route-cleanup gate. It launches an isolated `remote_headed` browser, seeds and checks out a route-pool entry, restarts the stream daemon, proves a healthy route is not repairable, kills the browser process, runs `service reconcile`, then dry-runs and applies `service_route_pool_repair` while saving service-state and repair artifacts under `/tmp/agent-browser-rdp-guac-route-cleanup-<timestamp>/`.

`service_route_pool_repair` returns stale checked-out route-pool entries to `available` and releases the stale remote-view route plus display-allocation records tied to those entries, so agents should use it before hand-editing retained route state after a browser crash.

Run `pnpm test:rdp-guac-cold-restart-readback-live` for the Slice J cold restart readback gate. It uses the current Guacamole route-pool readiness report, launches one isolated `remote_headed` browser, checks out one Guacamole-backed route, restarts the stream/control daemon, then verifies the route-pool entry, remote-view route, display allocation, browser, and Guacamole connection IDs still agree before and after reconcile.

Run `pnpm test:rdp-browser-reattach-until-close-live`, `pnpm test:rdp-two-route-switching-live`, `pnpm test:remote-view-reconcile-reattach-live`, and `pnpm test:dashboard-rdp-reattachable-rail-live` for P66 RDP browser reattachability. They require two ready Guacamole route-pool entries, launch two `remote_headed` browsers with distinct runtime profiles, park and reassign route surfaces without closing browsers, prove reconcile keeps parked browsers reattachable, and write artifacts under `/tmp/agent-browser-p66-rdp-reattachment-<mode>-<timestamp>/`.

The RDP gateway readiness smoke now prints compact readiness components with status, evidence, next action, and recovery copy for `guacd`, `xrdp`, `xrdp-sesman`, backend TCP, the Guacamole web route, dashboard-auth follow-up, iframe embedding follow-up, and public ingress follow-up. Launcher and workspace viewport readiness copy should use that shape when available so operators see auth, provider, browser, viewer ownership, and retained focus or takeover job failures as distinct next actions.

Run `pnpm test:rdp-guac-route-pool-readiness` before the Slice H many-to-many gate to inspect the local Guacamole database without printing passwords. It checks the Guacamole Compose containers, lists redacted RDP connection metadata, probes the Guacamole web route, checks guacd-to-RDP TCP reachability for the selected route candidates, requires at least two distinct route candidates by default, and emits a copyable `AGENT_BROWSER_RDP_ROUTE_POOL_JSON` value when the provider has enough ready routes. Emitted route-pool entries include redacted target identity and readiness metadata, not passwords. Pass `--report-only` to collect the JSON readiness report without failing the command on a one-route workstation. Run `pnpm setup:rdp-guac-route-pool -- --dry-run` to review the first static host-XRDP route-pool bootstrap without changing the host. Run `pnpm install:privileges -- --dry-run` to review the one-time privileged helper install, then `pnpm install:privileges -- --apply` from an interactive terminal to create the `agent-browser` group, install the root-owned helper under `/usr/local/libexec/agent-browser`, add the operator user to the group, and install the narrow sudoers rule. Open a new shell or run `newgrp agent-browser` after applying it. Then run `pnpm setup:rdp-guac-route-pool` only after the doctor or display inspector proves the existing route topology collapsed to one display, or when a reviewed operator override passes `--force`. It creates or updates two local XRDP users, creates or updates two Guacamole RDP connections, grants Guacamole read permission, stores generated XRDP passwords under the user-scoped Guacamole secret file, restarts XRDP, and then tells you to rerun the route-pool readiness smoke. The setup command requires the installed passwordless helper, fails closed when bootstrap is incomplete, and does not print the generated passwords. When route-specific Linux users and secrets already exist, use `pnpm sync:rdp-guac-route-specific-user-pool` instead of the sudo setup. It migrates the exact supported legacy connection rows in place, configures distinct route usernames, removes stale color-depth isolation metadata, and fails closed on mixed or duplicate managed rows. The old `pnpm sync:rdp-guac-existing-user-route-pool` spelling is a compatibility alias to this safe migration. Route readiness prefers current display names inferred from live Xorg sessions over persisted display hints. This bootstrap only creates distinct RDP sessions; P03 is complete only after the many-to-many live gate proves Browser A and Browser B are actually visible through those routes at the same time. After opening both route sessions, run `pnpm inspect:rdp-route-displays` to map the route users to active XRDP display names and print `AGENT_BROWSER_RDP_ROUTE_A_DISPLAY_NAME` and `AGENT_BROWSER_RDP_ROUTE_B_DISPLAY_NAME` for the live gate. Pass `--shell` to the display inspector or route-pool readiness smoke when you want copyable `export ...` lines instead of JSON. If the agent user cannot launch onto those XRDP-owned displays, run `pnpm grant:rdp-route-display-access -- --dry-run` to review the narrow local X grants, then run `pnpm grant:rdp-route-display-access -- --apply`. Apply requires the installed passwordless helper and never falls back to direct sudo as a route user. Run `pnpm test:rdp-guac-many-to-many-live` for the full Slice H gate. It requires two distinct route-pool entries through `AGENT_BROWSER_RDP_ROUTE_POOL_JSON` or paired `AGENT_BROWSER_RDP_ROUTE_A_*` and `AGENT_BROWSER_RDP_ROUTE_B_*` environment variables, plus at least `AGENT_BROWSER_RDP_TEST_CLIENT_A_EXECUTABLE` for dashboard viewers. Route entries may include `target.displayName` in the JSON pool, or `AGENT_BROWSER_RDP_ROUTE_A_DISPLAY_NAME` and `AGENT_BROWSER_RDP_ROUTE_B_DISPLAY_NAME` for paired environment variables. When display targets are present, the smoke launches each browser directly on that route's XRDP display with `displayIsolation="shared_display"` and requires the two display names to be distinct. When display targets are absent, it uses the service private-display allocator. The smoke launches two `remote_headed` browsers, checks out distinct Guacamole/RDP routes, records observer and controller leases for both routes, opens `view=workspace:tile` in two dashboard clients, refreshes Browser A's tile, closes Browser A, verifies Browser B remains ready, crops each dashboard iframe from tile screenshots, runs OCR against the remote-view pixels to prove route A shows Browser A and route B shows Browser B, and saves artifacts under `/tmp/agent-browser-rdp-guac-many-to-many-<timestamp>/`.

Run `pnpm test:mcp-read-no-launch` to validate that MCP resource reads remain read-only under the same launch defaults.

When manual seeding blocks an access-plan tab recipe, `serviceRequest.request` carries `blockedByManualAction` and `manualSeedingRequired`; HTTP `POST /api/service/request`, MCP `service_request`, and the client helpers reject that request unless `allowManualAction: true` is supplied. When CDP-free posture blocks the recipe, `serviceRequest.request` carries `requiresCdpFree: true` and `cdpAttachmentAllowed: false`; do not force the request through a CDP-backed tab path. Read `serviceRequest.cdpFreeAvailability` before launch to see that `cdp_free_launch` is the lifecycle-only alternative and which service-request actions remain unavailable without CDP. Use `cdp_free_launch` only when process lifecycle and service-state tracking are sufficient. Its response includes `unsupportedCommands`, a machine-readable list of service-request actions that still require CDP. Software clients can call `summarizeServiceCdpFreeLaunchAvailability()` or `isServiceCdpFreeActionAvailable()` before exposing follow-up controls. When CDP is allowed, request a service-owned tab first, extract the valid `serviceTabHandle`, then call `requestServiceCdpAttach()` with `cdpAttachmentAllowed: true`. The attach helper rejects missing, stale, CDP-free, and policy-blocked handles before posting the request. Use `requestServiceExternalByopAdopt()` or `adoptExternalByopBrowser()` only when a client has already registered an `external_byop` profile and can supply exactly one explicit `cdpUrl` or `cdpPort` for an already-running Chrome lane. That action records the lane as retained `host: "attached_existing"` service state, returns a `serviceTabHandle`, leaves cleanup as detach, and must not be used for `external_observed` profiles. Run `pnpm test:service-external-byop-adopt-live` when changing this path. Use `requestServiceEvaluate()` for bounded JavaScript reads against that handle; it requires positive `timeoutMs` and `maxReturnBytes` and returns URL/title plus result truncation metadata. Use `requestServiceDiagnostics()` when a client needs compact evidence for the same handle, including optional screenshot capture. Call `requestServiceCdpDetach()` when finished; detach records service trace evidence and preserves the browser process by default.

Use `agent-browser service reconcile` to run persisted browser health and target probes intentionally without requesting a control-plane status snapshot. It updates `~/.agent-browser/service/state.json`, expires due or orphaned service session leases without launching a browser, refreshes live tab records for reachable browser CDP endpoints, and returns the reconciled service state plus total and changed browser counts. Reconcile responses include `expiredSessionLeases`, `expiredSessionLeaseCount`, and `remoteViewRepair` counts for route-pool entries invalidated or restored from live X11 socket evidence, stale route checkouts released after their owner becomes orphaned, plus orphaned remote-view displays, routes, viewer leases, and controller leases repaired during reconcile. The persisted service state includes a `reconciliation` snapshot with `lastReconciledAt`, `browserCount`, `changedBrowsers`, and `lastError`, bounded recent control-plane job records in `jobs`, a derived `incidents` collection grouped by browser or service scope, and a bounded `events` log for reconciliation summaries, browser health transitions, browser recovery starts, profile lease wait transitions, ownership repairs, and tab lifecycle changes.

Use `agent-browser service prune-retained` only when retained service inventory itself is noisy after many closed jobs. It is dry-run by default and removes nothing unless `--apply` is present. The default candidate set is closed tabs plus inert `not_started` browser records with no PID, CDP endpoint, active sessions, view streams, or non-closed tabs. Add `--process-exited-browsers` only after reviewing failure evidence, because process-exited and unreachable records may explain a crash, close failure, or host problem. With `--abandoned-sessions`, that explicit flag can also remove old failed retained session lanes that have no retained tabs. Add `--orphaned-profiles` to include `custom:*` profile records with no retained service references and missing ephemeral user-data directories, plus unreferenced nonpersistent `managed_one_time` profile records. Add `--display-allocations` to classify retained display allocation records and prune only apply-safe orphan, stale-route, or historical-placeholder allocations.

Add `--released-sessions` to prune released or expired session records only when all linked browsers are inert `not_started` placeholders and the session has no retained tabs. Add `--abandoned-sessions` only after operator review because it applies the same inert-placeholder cleanup to shared or exclusive session leases that may otherwise affect profile allocation. When `--process-exited-browsers` is also present, abandoned-session cleanup can prune old linked `process_exited` or `unreachable` browser records that have no retained tabs, even when stale failure or view-stream metadata is still attached. Abandoned shared or exclusive sessions must also have a parseable `lastLeaseObservedAt` or `createdAt` older than `--abandoned-session-min-age-minutes`, which defaults to `1440`. Dry-runs report skipped abandoned sessions as `abandonedSessionsMissingAgeTimestamp` or `abandonedSessionsTooFresh`; treat those as retained lease evidence to review, not as safe cleanup candidates. Use `skippedSummary` and the text output skipped group lines to triage the top repeated job prefixes before deciding whether older retained sessions need migration or manual cleanup.

`agent-browser service status` includes `retainedDisplayAllocations`, a compact classification summary for retained display-allocation rows. Treat those as historical service-state records unless their class is `live`; use the included cleanup command for a dry-run before applying compaction.

Use `agent-browser service repair-retained` before abandoned-session cleanup when legacy inert session placeholders lack parseable age evidence. It is dry-run by default and only mutates state with `--apply`. The default repair stamps the current observation time into `lastLeaseObservedAt` for shared or exclusive sessions that have no retained tabs and only inert self-linked `not_started` browser placeholders. Repaired sessions become too fresh for `--abandoned-sessions` pruning until the minimum age guard elapses.

The Service dashboard exposes the retained-state cleanup workflow for operators. It queues `service_prune_retained` and `service_repair_retained` through `POST /api/service/request`, shows dry-run candidate counts including orphaned custom and managed one-time profile candidates, and only enables apply after the latest dry-run result is visible for review.
...
Use `agent-browser service trace --service-name <name> --task-name <name>` to inspect related events, jobs, incidents, and normalized activity in one response. Add `--limit <n>`, `--browser-id <id>`, `--profile-id <id>`, `--session-id <id>`, `--agent-name <name>`, or `--since <timestamp>` to narrow a trace view for one service, agent, task, browser, profile, session, or time window. Prefer this command when a client needs a complete service/task timeline without issuing separate jobs, incidents, events, and incident activity requests. The response includes a `summary` object with compact service, agent, task, browser, profile, and session context rows plus per-context record counts, target identity hints, control-plane modes, lifecycle-only job counts, naming warnings for missing service, agent, or task labels, and `summary.contexts[].attention` intervention metadata when incidents or missing labels need follow-up. Attention metadata is UI-neutral and uses `presentation: "client_decides"` so clients choose whether to log, prompt, show a dashboard affordance, or raise another operator channel. Software clients can call `summarizeServiceTraceAttention()` from `@agent-browser/client/service-observability` on a `getServiceTrace()` response to roll up required operator follow-up, service labeling follow-up, reasons, suggested actions, messages, and affected contexts without copying dashboard logic. Software clients can also call `createServiceTraceHandoff()` to produce the same copyable CLI command, HTTP path or URL, and MCP `service_trace` arguments that the dashboard shows for one trace filter set. Use `createServiceIncidentHandoff()` for the matching service incidents helper, including incident detail and activity references when an incident id is known. `summary.contexts[].targetServiceIds` lists the normalized target-service, site, and login identity hints observed on retained jobs in that context; text output shows them as `targets=...`. `summary.contexts[].controlPlaneModes` lists `cdp`, `cdp_free`, or `service`, and `lifecycleOnlyJobCount` counts CDP-free launch and service lifecycle jobs that are not CDP-backed tab interactions. `summary.browserCapabilityLaunches` lists compact launch binding decision rows from retained launch events or matching service sessions, including applied or skipped state, reason, browser build, binding ID, host ID, executable ID, and context labels. Text output includes a `Browser capability launches` block when those decisions are present, and the dashboard trace explorer renders the same rows as operator cards. `summary.displayAllocations` aggregates requested private, shared, and ambient display allocation across retained jobs in the trace, and each context also reports its observed `displayAllocations` plus `unrecordedDisplayAllocationJobCount`; software clients can call `summarizeServiceTraceDisplayAllocations()` to roll those fields into counts, compact labels, unrecorded job ids, and affected contexts without copying dashboard grouping logic. In the dashboard, display-allocation summary cards jump to the Jobs workspace with the matching display filter applied. `summary.profileLeaseWaits` provides a per-job profile lease wait rollup with outcome, timing, conflict sessions, and trace labels. Text output includes a `Profile lease waits` block when that summary or the raw wait events are present. In the dashboard, profile lease wait cards and job-backed timeline rows jump to the Jobs workspace with the retained job id filter applied, incident-backed timeline rows jump to the Incidents workspace with the retained incident id filter applied, and incident detail actions derive trace filters from retained incident context before loading the broader trace in Events. The dashboard also renders copyable CLI and HTTP handoff strings for the active trace filters so an operator can paste the same query into another agent, terminal, or software client. For crash recovery, read the `events` array in order: `browser_health_changed` with `currentHealth` such as `process_exited` or `cdp_disconnected` and `details.currentReasonKind`, `browser_recovery_started` with `details.reasonKind`, `details.reason`, `details.attempt`, `details.retryBudget`, `details.nextRetryDelayMs`, and `details.policySource`, then `browser_health_changed` with `currentHealth: "ready"` after relaunch. Stale health and recovery events also include `details.failureClass`, such as `browser_process_exited`, `cdp_unresponsive`, `cdp_endpoint_unreachable`, or `target_discovery_failed`. Process-exit health and recovery events also include `details.processExitCause: "unexpected_process_exit"`. Active local Chrome process exits include `details.processExitDetection: "local_child_try_wait"`, `details.processExitPid`, `details.processExitCode` or `details.processExitSignal` when available, and `details.browserStderrLogPath` when Chrome stderr was captured under `~/.agent-browser/tmp/chrome-launches/`. Browser launch events include `details.browserStderrLogPath` when Chrome stderr capture is available. CDP-disconnect health events also include `details.browserStderrLogPath` when agent-browser owns the local Chrome process. Operator-requested shutdown health events instead include `details.shutdownReasonKind: "operator_requested_close"` and `details.processExitCause: "operator_requested_close"` plus polite-close and force-kill outcome flags, so agents can separate clean or degraded closes from unexpected exits. If the next attempt would exceed the configured retry budget, the browser is marked `faulted` and the command fails instead of relaunching. Operators can use `service retry <browser-id>` to record a `browser_recovery_override` event and allow a new recovery attempt. HTTP clients get the same sequence from `/api/service/trace`; MCP clients get it from the `service_trace` tool.

Profile selection hints can include `targetServiceId`, `targetServiceIds`, `siteId`, `siteIds`, `loginId`, `loginIds`, `accountId`, `accountIds`, or `url`. The selector prefers `authenticatedServiceIds`, then `accountIds`, then `targetServiceIds`, then `sharedServiceIds`; `url` derives a known target identity from the matching site-policy origin.

Use MCP `service_request` when a service wants to queue one intent-based browser action with caller context, site or login hints, explicit `profile` or `runtimeProfile` hints, optional top-level `browserId` or `sessionName` reuse route hints, `action`, `params`, and `jobTimeoutMs` in a single request object. This is the service-oriented equivalent of `browser_command` for clients that want agent-browser to own profile selection, queueing, and trace metadata deterministically. Top-level `browserId` and `sessionName` route ordinary commands to an existing retained daemon lane; action-specific `params.browserId` and `params.sessionName` remain scoped to actions such as `view_takeover`. Use `allowDuplicateProfileLane: true` only for reviewed isolation or throwaway browser behavior.

For v2 broker-issued task authority, copy the exact next broker-assigned
`approvedPlan.steps[usage.nextStepIndex].stepId` into top-level `taskStepId`.
Omit `consequenceCeiling` to preserve the default read/navigation issuance
posture. When an exact approved plan requires page mutation, file transfer, or
bounded evaluation, request the smallest matching ceiling; the broker accepts
explicit ceilings only through `script_execution` and still restricts execution
to the plan's exact action allowlist, target, URLs, expiry, and evidence budget.
Do not choose, repeat, skip, or reorder step IDs. Admission binds the step to
its action, exact pre-action and requested URLs, evidence reservation, command
ID, target, and immutable plan hash, then persists the receipt before dispatch.
Before response publication, the daemon durably finalizes that receipt as
`completed` or `failed` with the exact response digest and post-action target
evidence. An admitted receipt without terminal evidence is reported as
`indeterminate` after a crash and remains consumed. Read authority status and
continue only from its reported next step. For exactly one indeterminate step,
inspect it and use the confirmed broker reconciliation operation to obtain a
lineage-bound replacement plan. Never replay a consumed step or edit an issued
plan in place.

Task-authority HTTP issue, reconcile, revoke, and confirmation mutations use
the authenticated dashboard superuser as their durable requester or decider.
MCP derives the actor from the OS-owned stdio transport. Do not invent these
identity fields. For terminal confirmation receipt retention, preview with
`service_task_authority_confirmation_cleanup`, review its exact digest, and
only then apply with that digest. Pending and indeterminate receipts are never
cleanup candidates. Review the returned verified ledger head, active digest,
segment capacity, segment count, and candidate receipt hashes before apply.
Retired IDs remain exactly single-use in a bounded active manifest plus
fixed-capacity immutable hash-chained segments; any integrity mismatch fails
closed.

Retained service jobs preserve target identity hints for profile debugging. Singular `targetServiceId`, `siteId`, and `loginId` fields keep exact singular request values when present, and `targetServiceIds` stores the normalized target-service, site, and login identity set used for profile selection. Jobs also include `controlPlaneMode` (`cdp`, `cdp_free`, or `service`), `lifecycleOnly`, and requested remote-headed `displayIsolation` so callers can distinguish CDP-free launch, service lifecycle jobs, and private, shared, or ambient display allocation from CDP-backed tab interactions. Prefer these fields in `service jobs`, HTTP jobs, MCP `agent-browser://jobs`, and service trace output instead of reconstructing identity, attachment posture, or display ownership from the original command payload.
...
Use `examples/service-client/` as the copyable software-client workflow for access-plan-first `requestServiceTab`, trace inspection, trace and incident handoff generation, and optional known queued-job cancellation with `cancelServiceJob`. Run `pnpm test:service-client-example` to validate the no-launch broker-first example contract plus dry-run modes, or `pnpm test:service-client-example-live` to validate it against an isolated live daemon and browser session. The generic `service-request-trace.mjs` example is the recommended non-Canva integration path: pass `--register-profile-id` and `--register-readiness-monitor` when a service may need a recurring managed profile, then use `acquireServiceLoginProfile()` so agent-browser gets the access plan first, registers the profile only when no selected profile exists, adds the retained freshness monitor for that fallback profile, refreshes the access plan, and submits the planned tab request. Pass `--run-due-readiness-monitor` when the example should execute an access-plan-recommended freshness monitor before browser work. Pass `--run-browser-capability-preflight` when the example should execute the final access plan's no-launch browser-capability gate before browser work. The example output includes `profileAcquisitionSummary.monitorRunDueRan`, `profileAcquisitionSummary.browserCapabilityPreflightRan`, `browserBuildSelectionSummary`, `initialRecommendedAction`, `refreshedRecommendedAction`, monitor-run target freshness fields, browser-capability preflight outcome fields, a `handoff` object from `createServiceTraceHandoff()` and `createServiceIncidentHandoff()`, and latest trace job `controlPlaneMode` plus `lifecycleOnly` values for operator inspection. The same package includes `managed-profile-flow.mjs`, a CanvaCLI-style profile-broker recipe that uses `getServiceAccessPlan()` to inspect the selected profile, readiness, seeding handoff, monitor findings, site policy, providers, retained challenges, and service-owned decision before registering a managed login profile only when agent-browser has no suitable one, optionally adding a profile-readiness monitor with `upsertServiceProfileReadinessMonitor()`, optionally running an access-plan-recommended due monitor with `--run-due-readiness-monitor`, refreshing the access plan, optionally running the no-launch browser-capability gate with `--run-browser-capability-preflight`, and passing the final response to `requestServiceTab()` or `requestServiceCdpFreeLaunch()` when the selected site policy requires CDP-free operation. It can post bounded auth-probe evidence with `updateServiceProfileFreshness()` for an existing profile. When readiness requires manual seeding, the recipe returns a skipped tab result with `reason: "manual_seeding_required"` instead of posting a doomed request. Its output includes `profileAcquisitionSummary`, `browserBuildSelectionSummary`, due-monitor target freshness fields, browser-capability preflight outcome fields, `readinessSummary.needsManualSeeding`, target service IDs, recommended actions, `cdpFreeAvailability` for the pre-launch headed no-DevTools branching decision, and `seedingHandoff` when readiness says an operator must seed the profile. Run `pnpm test:service-client-managed-profile-flow` for the no-launch mock smoke that proves an existing managed profile is selected without registering a new one and can run due monitors and browser-capability preflights before browser work. Run `pnpm test:service-client-managed-profile-flow-live` when you need the same Canva-style due-monitor path validated against an isolated daemon and browser.

For software-client profile selection, prefer requesting the target identity instead of naming a browser profile. A service profile with `authenticatedServiceIds: ["acs"]`, `targetServiceIds: ["acs"]`, `accountIds: ["eric@example.com"]`, and `sharedServiceIds: ["JournalDownloader"]` lets `requestServiceTab({ serviceName: "JournalDownloader", loginId: "acs", accountId: "eric@example.com", ... })` select the profile with usable ACS login state. A client such as CanvaCLI should use the same pattern: request `serviceName: "CanvaCLI"` plus `loginId: "canva"`, `targetServiceId: "canva"`, `accountId`, or `url` and let agent-browser pick or reuse the managed profile. If no managed profile is suitable, register one with `registerServiceLoginProfile({ id: "canva-default", serviceName: "CanvaCLI", loginId: "canva", authenticated: false, keyring: "basic_password_store" })`, add `upsertServiceProfileReadinessMonitor({ serviceName: "CanvaCLI", loginId: "canva" })` for recurring profile freshness, have the operator seed it when readiness reports `needs_manual_seeding`, then request tabs by login identity rather than by profile name. For Google, Gmail, Chrome sync, passkeys, and browser plugin setup, the first seeding launch must be headed and detached without `--attachable` or a CDP/DevTools port. The selector prefers `authenticatedServiceIds`, then `accountIds`, then `targetServiceIds`, then `sharedServiceIds`. Retained session records expose `profileSelectionReason` as `authenticated_target`, `account_match`, `target_match`, `service_allow_list`, or `explicit_profile`, `profileLeaseDisposition` as `new_browser`, `reused_browser`, or `active_lease_conflict`, and browser launch events mirror both in `details`. Active exclusive profile conflicts are rejected before a service-scoped launch starts another browser unless `profileLeasePolicy: "wait"` is supplied with a bounded `profileLeaseWaitTimeoutMs`. Use explicit `profile` or `runtimeProfile` only for override workflows where the caller intentionally owns the browser identity choice.

Some services are bot-sensitive enough that they may not even load when Chrome has a CDP or remote debugging port enabled. Treat Canva-class sites as CDP-free operating-mode candidates, not as ordinary attachable runtime profiles. Access-plan `decision.launchPosture` reports `browserBuild`, `browserBuildSource`, `requiresCdpFree`, and `cdpAttachmentAllowed`; if `requiresCdpFree` is true, do not ask the operator to use an attachable profile or open a DevTools port. The built-in Canva site policy recommends `cdp_free_headed` by default. Sites can use `stealthcdp_chromium` when CDP-backed control remains acceptable but a patched Chromium executable is preferred. UPS is the current example of that posture: CDP-backed headed stealth Chromium is acceptable, but true headless is known-bad on this workstation, so use the access-plan headed recommendation rather than overriding it with `headless: true`. Current agent-browser support can still own the headed browser process, profile lease, lifecycle state, and operator handoff, but CDP-backed commands may be unavailable until non-CDP observation and control primitives are implemented. Read `unsupportedCommands` from the `cdp_free_launch` response, or use `summarizeServiceCdpFreeLaunchAvailability(response.data)`, before attempting follow-up automation.
...
the access plan's `browserId` and `sessionName` route hints. The MCP adapter
uses those fields only to select an existing retained daemon lane. It does not
launch or borrow a duplicate profile process, and conflicting or opaque route
...
Use `agent-browser service resources` before manual process cleanup when browser hosts are consuming too many resources. It builds a no-launch process inventory for agent-browser daemons, browser processes, and remote display processes, then correlates each row with retained service browser IDs, profile IDs, session IDs, display allocation IDs, profile paths, and CDP ports when possible. The classifier protects the dashboard service main PID, retained active browser PIDs, and named or persistent managed profiles. Resource warnings include `duplicate_live_browsers_for_profile` and `duplicate_active_profile_leases`; `agent-browser install doctor` reports those as `service_duplicate_profile_pressure`. Use `agent-browser service gc --dry-run` to group conservative cleanup candidates and receive a short-lived review token. Use `agent-browser service gc --apply --review-token <token>` only after reviewing that exact dry-run. Apply mode re-reads process identity before termination, sends SIGTERM first, uses SIGKILL only for a still-matching candidate, and persists compact service-event counts. Use `--force-without-review` only for explicit operator override. The optional read-only timer is installed with `scripts/install-resource-monitor-user-timer.sh`, removed with `scripts/remove-resource-monitor-user-timer.sh`, writes only aggregate status to `~/.agent-browser/service/resource-monitor-summary.json`, and can be read with `agent-browser service resources --monitor-summary`.
...
exact session/profile/process/CDP proof, observed process exit and profile-lock
release. This recovered Linux Chrome path has no PID-signal fallback. Refused or
unconfirmed shutdown preserves recovery records; never force attachment or delete
records to bypass a refusal.
...
Service control jobs do not time out at the worker boundary by default. Set `service.jobTimeoutMs`, pass `--service-job-timeout <ms>`, or set `AGENT_BROWSER_SERVICE_JOB_TIMEOUT_MS` to mark long-running dispatched jobs as `timed_out`. Use `0` to disable it. For one ordinary CLI command, pass `--job-timeout-ms <ms>` so the daemon cancels that command and releases its serialized queue before any longer caller-side subprocess deadline expires. Never retry a timed-out renderer action automatically. The daemon probes browser-level CDP after returning the timeout, replaces only an affected ordinary owned tab with a ready blank target, and performs one bounded locally owned browser recovery if CDP is lost. It preserves service-tab handles and externally attached browsers for explicit broker or operator reconciliation.
...
The dashboard runs independently of browser sessions on port 4848 (configurable with `--port`). All sessions automatically stream to the dashboard. The dashboard has an app-owned login screen and protects its operator APIs with superuser sessions. On first start it creates `~/.agent-browser/dashboard-auth.json` and writes bootstrap credentials for the default `admin` superuser plus the `codex` observer to the mode-0600 file `~/.agent-browser/dashboard-auth.env`; `~/.agent-browser/.env` records `AGENT_BROWSER_DASHBOARD_AUTH_FILE` so foreground and user-service runs share the same credential store. Dashboard sections are URL-addressable: use `/service`, `/browsers`, and `/activity` for direct links. Service record tabs persist in `/service?view=service:<name>` with `service:browsers`, `service:profiles`, `service:incidents`, `service:sessions`, `service:tabs`, `service:jobs`, or `service:events`, and the default tab is `browsers` so managed browser rows are visible immediately. The left workspace navigator separately persists selected workspace identity in `workspace`, `browser`, `session`, `tab`, `profile`, and `job` query parameters. On Linux hosts where ingress depends on a stable dashboard listener, run `bash scripts/install-dashboard-user-service.sh` from the repo to install and start a user-scoped `agent-browser-dashboard.service` plus `agent-browser-runtime-interlock.timer`; set `AGENT_BROWSER_DASHBOARD_PORT` first when the listener should use a non-default port, or set `AGENT_BROWSER_RUNTIME_INTERLOCK_INTERVAL` to another systemd duration when the default five-minute post-completion health cadence should change. The interlock hands doctor-proven stale daemon sessions to the current executable without closing their browsers, reconciles retained state, recreates or migrates the supported Guacamole route fixtures to distinct route-specific users when doctor requests missing-fixture or collapsed-display repair, reruns both doctors before display restoration, restores missing Guacamole/XRDP route displays, reapplies display access when required, and writes `~/.agent-browser/convergence/local-runtime-latest.json` without replacing installed artifacts. Fixture repair uses route-specific user-scoped XRDP secrets and fails closed on a partial schema, missing user or secret, ambiguous managed rows, or unsupported topology; it is not a PostgreSQL backup or restore. The standalone dashboard prefers a live `dashboard-service-backend` session for proxied `/api/service/*` calls before falling back to `default` or the first discovered session, which keeps public ingress on a current backend without disturbing an existing default browser. Dashboard source changes are visible on the long-running local dashboard only after the exported bundle is embedded in the installed binary and `agent-browser-dashboard.service` is restarted; run `pnpm publish:local-dashboard -- --expect-marker "Stream port"` to hand active sessions to replacement daemons while preserving browser PIDs, CDP endpoints, profiles, and tabs; the publisher fails before executable replacement if an active older daemon cannot support the handoff. When no user systemd bus is available, it validates exact PID, user, command, and dashboard-mode evidence before retiring a standalone dashboard listener; identity mismatch fails closed. Required HTTP, marker, and runtime-manifest readiness is committed before disposable-browser QA. A known pre-render browser launch failure is reported as `browser_launch_unavailable` without invalidating proven readiness; authentication, DOM, workspace, and other post-launch failures remain fatal. Add `--require-browser-smoke` when disposable-browser launch must be a required gate, or `--skip-browser` for HTTP-only publication. Then tell the operator to check `http://127.0.0.1:4848`, and run `pnpm smoke:local-dashboard-runtime -- --expect-marker "Stream port"` to verify the already installed service. The smoke reads `/api/runtime/manifest` and records the live package version, `service-ui-runtime.v1` contract, embedded dashboard SHA-256, supported UI features, current executable path, and current executable SHA-256; the publish command fails if that manifest does not match the just-installed binary. `agent-browser install doctor --json` and `agent-browser doctor remote-view --json` also include `dashboardRuntime` so the CLI doctor and live dashboard manifest describe the same installed binary and embedded UI bundle. If the dashboard shows `Runtime contract drift`, publish the local dashboard runtime again before trusting operator-visible behavior. Sessions can also be created from the dashboard UI with local engines or cloud providers. Use the left workspace navigator to scan actionable Agent-browser owned and Detected non-owned browser groups derived from service-owned browser, detected non-owned browser, session, tab, profile allocation, job, incident, and view-stream state, then launch supported browser/profile combinations as their own daemon sessions or open a controllable embedded viewport when stream metadata allows it. Attention, stale, and retained records belong in Service, trace, event, job, incident, and log viewers rather than the live left rail. Detected browsers that are not agent-browser owned are explicitly labeled as non-owned and are not mixed into the owned group. For a detected non-owned browser with a reachable page CDP target, use Screenshot for a PNG, Watch live for a responsive image feed, and Borrow control for a five-minute pointer, keyboard, and wheel grant. Release ends the grant early. Borrow never enables Close, Kill, profile release, or lifecycle ownership. RDP gateway rows require current operator-visible browser-window proof before View, Control, or external open become available; terminal-only, idle-display, or missing-proof route rows stay visible as disabled diagnostics instead of opening a generic desktop as if it were the browser. Row selection opens a dense right-pane Workspace inspector with a compact status strip, PID/RSS/CPU/CDP/stream indicators, action availability with reasons, page identity, ownership, viewport readiness, route-proof state, and a collapsed diagnostic Evidence disclosure. Live daemon sessions publish a `cdp_screencast` stream from their runtime stream port, so rows such as `127.0.0.1:<port>/app` can render directly even when no service-owned browser record exists. Guided launches preserve the access-plan selected `runtimeProfile`, configured profile path, `browserBuild`, and executable evidence, and client-decided freshness warnings do not disable Launch when the launchable request is available. The launcher defaults to the shared remote desktop display, RDP gateway view stream, and manual desktop input; selecting RDP gateway starts a fresh `remote_headed` session so the launched browser appears in the embedded Guac viewport when `AGENT_BROWSER_REMOTE_VIEW_URL` and `AGENT_BROWSER_REMOTE_HEADED_DISPLAY` are configured. When stream metadata is embeddable and operator-visible, the dashboard closes the launcher, queues `view_focus` with target ID plus tab-index fallback when both are known, and opens the embedded workspace viewport; otherwise it falls back to Service Jobs or keeps the row disabled with the route-proof reason. Blocked or human-takeover rows remain in Attention because automation or lease progress is blocked, but Control or View stays the primary operator action whenever the related browser still exposes a controllable service-owned stream; disabled Resume remains visible until a backend release action exists. Use the Service view to inspect worker/browser health, set a remembered operator identity for incident audit metadata, add optional operator notes when acknowledging or resolving incidents, inspect prominent service-sourced incident severity, escalation, recommended action displays, and remedy summary groups, inspect the service-owned incident history timeline with local fallback, load combined service, agent, task, browser, profile, session, and time-window traces from `/api/service/trace`, including ownership summary cards, naming warnings, copyable CLI and HTTP handoff strings, profile lease wait cards and timeline job rows that jump to their retained Jobs row, incident-backed timeline rows that jump to their retained Incidents row, incident detail actions that jump back to the related trace, display-allocation summary cards that jump to filtered Jobs rows, view a browser-health transition timeline for crash/recovery visibility, inspect the backend-owned `profileAllocations` view and detail dialog for holder sessions, waiting jobs, conflicts, lease state, and recommended actions refreshed from `GET /api/service/profiles/<id>/allocation`; run `pnpm test:dashboard-profile-allocation` when changing that detail lookup. Use a grouped incident browser panel with handling-state filters plus acknowledge and resolve actions, inspect incident filtering for crash/disconnect/recovery and timed-out or cancelled jobs, check reconciliation status, review managed entity counts, inspect service jobs with text, state, display-allocation, sort, and row-limit controls, select managed browser, profile allocation, incident, session, tab, and job rows into the desktop right-pane inspector or mobile detail dialog, use the selected-record hero summary, primary action bar, runtime/control/ownership/activity sections, related-record jumps, and Evidence disclosure instead of scanning raw field cards, acknowledge or resolve selected incidents from the right pane, cancel selected queued or running jobs from the right pane, open embedded remote-view streams for browsers or service tabs with fullscreen and Guacamole interaction settings controls, rely on queued `view_focus` to bring the selected tab forward and request native-window maximize before inspection, inspect event details, and run reconciliation from the UI.

Use `pnpm --silent converge:local-runtime -- --json` to preview local runtime
convergence. Repairable nonzero doctor output remains visible in
`safeRemedies`, and dry-run never executes those remedies.

For local dashboard rendered-page QA, the smoke resolves installed
`launchConfig` and selects patched Chromium only when manifest, executable,
artifact-smoke, and WSL profile-smoke evidence agree. It creates a disposable
profile under the matching Windows temp root, passes `--browser-build` through
agent-browser, and does not add a general `--no-sandbox`. Use
`--browser-profile` only as an intentional reviewed override.
Run `pnpm test:local-dashboard-publisher-lifecycle` after changing standalone
dashboard ownership, quiescence, normal restart, or rollback behavior. It uses
only isolated temporary processes and does not replace the live runtime. Run
`pnpm test:local-dashboard-publisher-orchestration` after changing top-level
build, backup, replacement, handoff, readiness, or rollback ordering. It uses
isolated real files and injected adapters without building, installing,
restarting, or attaching to a browser. Publication reports bind built, source,
backup, replacement, and restored binaries to SHA-256 evidence. Backup mismatch
fails before quiescence. Restore failure remains secondary to the initiating
publication error, and restart occurs only when the current binary still
matches a verified backup or verified replacement. Before quiescence, a secured
atomic transaction is written under `~/.agent-browser/publications/`. The next
invocation recovers an incomplete verified replacement to readiness or a
verified source to rolled back before any new build. Exact handoff descriptors
and already-resumed PID/CDP evidence prevent duplicate resume; unknown installed
bytes fail closed. Use `pnpm status:local-dashboard-publication` for strictly
read-only journal and artifact inspection. Use
...
When one retained browser is a required publication gate, pass
`--expect-retained-session`, `--expect-retained-profile`,
`--expect-retained-target`, and `--expect-retained-url`. The publisher reads
the exact CDP target before backup or quiescence, pins the browser PID and CDP
endpoint, then requires the same identity after handoff and at final readiness.
The journal carries that pin into recovery. Do not omit these flags and infer
preservation from an idle daemon close when exact retained-target proof is an
acceptance criterion. Use `pnpm test:rust-isolated` for a local full Rust suite
so tests run serially under a temporary home, agent root, socket directory, and
runtime directory instead of the operator's live state.
Use `pnpm check:local-dashboard-retained-browser --` with the same exact
identity flags when only a read-only, no-lock, no-build preflight is needed. A
missing daemon fails without launching a replacement session or browser.
Once the exact lane is live and verified, use
`pnpm pin:local-dashboard-retained-browser --` with the session, profile,
target, and URL flags. This writes a private stable-identity requirement under
`~/.agent-browser/publications/`; it excludes PID and CDP endpoint data. Future
publishes and `pnpm check:local-dashboard-retained-browser` load it
automatically and reject identity drift before build. An absent pin is
`not_configured`; a malformed or mismatched configured pin fails closed. The
source-checkout user-service interlock executes the same check before
convergence, so do not bypass a failed configured-lane watchdog by starting
another browser.
The installed runtime exposes the same bounded status in
`agent-browser install doctor --json` under `data.localDashboardPublication`,
authenticated HTTP `GET /api/service/publications/local-dashboard`, MCP
`agent-browser://publications/local-dashboard`, the generated
`getLocalDashboardPublicationStatus()` helper, and the Service dashboard.
These reads never acquire the publisher lock or authorize recovery. The
dashboard deliberately has no recovery POST or button; when status recommends
`recover_only`, review the transaction and artifact hash before explicitly
running `pnpm recover:local-dashboard-publication`.

An available profile tile provides **Open browser**, which opens the guided
launcher with that exact profile selected and runs its no-launch access-plan
preflight. Every live service-owned browser provides **Close** in the workspace
tree and Service browser table. Detected non-owned browsers retain screenshot,
watch, and borrowed-input controls without lifecycle ownership.

When one browser reports several view streams, the workspace viewport exposes
an explicit CDP or RDP source chooser and remembers the choice per browser.
Recoverable browser tiles with no usable stream remain visible with **Wake
stream**. That action reattaches or switches the retained route and must not
launch a duplicate browser process on the same profile.
...
## Operator Workshop Boot Recovery

If `agent-browser-workshop-recovery.timer` is installed, its separate operator
helper restores only the explicitly armed retained conversation after a new
kernel or WSL distribution start, identified by kernel boot ID plus PID 1 start
ticks. It is not a general session launcher or prompt scheduler. Changed artifacts,
identity, enforcement digest, pending attempt or stale lock require inspection.
Never delete its private state to bypass an uncertain attempt. A healthy same-boot
run is a no-action verification; a same-boot missing browser is refused. Installation
and rearming are explicit operator actions, not part of routine browser use.
Before a reviewed restart, run the installed helper with `--check-readiness` and
require dependencies ready plus retainedVerified=true. It checks the armed
artifact manifest; an installation update requires explicit reviewed rearming.
Recovery waits up to three minutes for local dependencies before recording any
potential browser effect. Each system service is checked independently. Timeouts
leave the attempt unused; permission failures stop without retrying.
Successful service completion remains `active (exited)` with
`RemainAfterExit=yes` and `KillMode=control-group` to retain recovered children.
This state is not live browser-health proof. Starting an already active service
does not rerun verification. Never restart it as a health check: stop/restart
terminates its children and same-boot recovery can then refuse. Disable only the
timer to prevent future recovery without terminating the current browser.
Failed commands expose only allowlisted diagnostic categories and exit metadata;
never print raw child output to recover hidden detail. An unclassified error is
not proof of a particular cause. Preserve pending receipts during diagnosis.
The desktop opener rejects unsuccessful launch/header/navigation JSON even when
the subprocess exits zero. `navigation_outcome_unknown` stops without retry or
display-only success. Reconcile before retrying. Unsupported authentication is
rejected before any viewer launch.

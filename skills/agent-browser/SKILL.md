...
installed payload and `--apply --json` materializes the installed binary,
versioned support manifest, pinned local Guacamole Compose stack and schema,
protected generated PostgreSQL credential, dashboard service, runtime
interlock, and PostgreSQL backup units. Treat `ready: false` as a hard stop and
do not activate those units until the substrate-provisioning packet is
complete.
